...
// 🌟 ตัวแปรรับค่า 4 ตัวพอดีเป๊ะ ไม่เกินกฎ
...
static int	process_group(t_dict *dict, int size, char *chunk, int groups)
{
	...
	if (!(chunk[0] == '0' && chunk[1] == '0' && chunk[2] == '0'))
	{
		...
		if (groups > 1)
		{
			...
			free(s_key);
			if (!suf) return (0);
			write(1, " ", 1); write(1, suf, ft_strlen(suf)); write(1, " ", 1);
		}
	}
	return (1);
}
...
void	run_normal_mode(t_dict *dict, int size, char *num_str)
{
	...
	{
		suf = find_in_dict(dict, size, "0");
		if (suf) write(1, suf, ft_strlen(suf));
		write(1, "\n", 1);
		return ;
	}
...
void	run_standard_input_mode(t_dict *dict, int size)
{
	...
	while (read(0, &buf, 1) > 0)
	{
		...
		{
			input[i] = '\0';
			if (check_invalid_input(input) != 0) // ตรวจว่าเลขถูกไหม (อิงจาก validate.c ของคุณ)
